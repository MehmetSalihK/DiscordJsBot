...
export default {
    name: 'rgbpanel',
    description: 'Affiche le panel de gestion des rôles RGB',
    ...
    async execute(message, args) {
        try {
            ...
            logger.info(`ℹ️ [INFO] Panel RGB affiché par ${message.author.tag}`);
            return message.reply({ embeds: [embed], components: rows });

        } catch (error) {
            logger.error('Erreur dans la commande prefix rgbpanel', error);
            const embed = new EmbedBuilder()
                .setColor('#ff0000')
