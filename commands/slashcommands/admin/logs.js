  ...
  async execute(interaction) {
    try {
      if (!interaction.memberPermissions?.has(PermissionsBitField.Flags.ManageGuild)) {
        return interaction.reply({ embeds: [createErrorEmbed('Permission insuffisante', "Vous n'avez pas la permission de gérer la configuration du serveur.")], ephemeral: true });
      }
      ...
      if (sub === 'enable' || sub === 'on') {
        if (!logs.logChannelId) {
          return interaction.reply({ embeds: [createErrorEmbed('Aucun salon configuré', 'Veuillez d\'abord définir un salon avec `/logs set #salon`.')], ephemeral: true });
        }
      ...
      return interaction.reply({ embeds: [createErrorEmbed('Erreur', "Une erreur est survenue lors de l'exécution de /logs.")], ephemeral: true });
    }
  },
};
