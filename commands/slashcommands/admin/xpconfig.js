    ...
    try {
      ...
      if (sub === 'enablelogs') {
        if (!x.xpLogs?.logChannelId) {
          return interaction.reply({ embeds: [createErrorEmbed('Aucun salon configuré', 'Veuillez d\'abord définir un salon avec `/xpconfig setlog #salon`.')], ephemeral: true });
        }
      ...
      return interaction.reply({ embeds: [createErrorEmbed('Erreur', "Impossible d'exécuter la commande xpconfig.")], ephemeral: true });
    }
  }
};
