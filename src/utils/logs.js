...
// Logs du système d'XP
export async function sendXPLog(client, guildId, embed) {
  try {
    const conf = getGuildConfig(guildId);
    const xpConfig = conf?.xpSystem?.xpLogs;
    
    // Vérifier si les logs XP sont activés
    if (!xpConfig?.active) {
      console.log(`[Logs] Logs XP désactivés pour le serveur ${guildId}`);
      return false;
    }
    
    // Récupérer l'ID du canal de logs XP
    const channelId = xpConfig.logChannelId;
    if (!channelId) {
      console.log(`[Logs] Aucun canal de log XP configuré pour le serveur ${guildId}`);
      return false;
    }
    ...
    console.error(`[Logs] Erreur dans sendXPLog pour le serveur ${guildId}:`, error);
    return false;
  }
}

// Logs des activités vocales
export async function sendVoiceLog(client, guildId, embed) {
  try {
    const conf = getGuildConfig(guildId);
    const voiceConfig = conf?.xpSystem?.voiceLogs;
    ...
    const channelId = voiceConfig.logChannelId;
    if (!channelId) {
      console.log(`[Logs] Aucun canal de log vocal configuré pour le serveur ${guildId}`);
      return false;
    }
